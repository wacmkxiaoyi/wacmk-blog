...
from apps.blog.utils.site import get_or_create_site_setting
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not site_setting.audit_log_cleanup_enabled:
            self.stdout.write("Audit log cleanup is disabled.")
            return

        cutoff_date = timezone.localdate() - timedelta(days=site_setting.audit_log_retention_days)
