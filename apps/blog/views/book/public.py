...
from apps.blog.utils import is_ajax_request, record_book_view
...
from apps.blog.views.post.utils import post_requires_password
...
class BookListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return queryset.annotate(post_count=Count("posts", distinct=True)).order_by("name")
...
class BookDetailView(LoginRequiredMixin, DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.method.lower() == "post" and self.object.visibility == Book.VISIBILITY_ENCRYPTED:
            return super().dispatch(request, *args, **kwargs)
        if can_access_book(request, self.object):
            return super().dispatch(request, *args, **kwargs)
        if self.object.visibility == Book.VISIBILITY_ENCRYPTED:
    ...
    def get_context_data(self, **kwargs):
        post = None if kwargs.get("requires_password") else self.get_current_post()
        context = super().get_context_data(**kwargs)
        ...
        if post is not None:
            ...
            context["requires_post_password"] = requires_post_password
        ...
        context["requires_password"] = kwargs.get("requires_password", False)
