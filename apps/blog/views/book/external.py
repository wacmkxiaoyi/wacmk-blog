...
from apps.blog.access.check import build_access_check
from apps.blog.visibility import book_has_any_conditions, get_book_access_icon_presentation, get_book_visibility_presentation
    ...
    def get_context_data(self, **kwargs):
        ...
        if requested_slug:
            ...
            if current_post is None:
                raise Http404
            if not can_display_post_in_book_navigation(current_post, self.request.user, is_share_view=True):
                raise Http404
        if current_post is None:
            current_post = get_first_visible_book_post(self.object, self.request, is_share_view=True)
        if current_post is None:
            raise Http404
        post_access_check = build_access_check(current_post, self.request.user, in_book_context=True)
        context["post_access_check"] = post_access_check
        if not post_access_check["all_granted"]:
            context["post_gate_check_url"] = reverse("access-check", kwargs={"object_type": "post", "object_id": current_post.pk})
